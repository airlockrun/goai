...
Thanks for considering a contribution to goai.

## License

goai is licensed under the [Apache License, Version 2.0](LICENSE). By submitting a contribution, you agree it will be made available under the same license.

This project is part of a multi-repo open-source effort alongside [airlock](https://github.com/airlockrun/airlock) (AGPL-3.0), [agentsdk](https://github.com/airlockrun/agentsdk), and [sol](https://github.com/airlockrun/sol). The companion repositories have their own CONTRIBUTING.md.
...
goai is a Go port of the [Vercel AI SDK](https://github.com/vercel/ai). Most updates here come from tracking upstream — see [NOTICE](NOTICE) for attribution.

We accept:

- **Bug fixes for the Go-specific port** — issues that exist in goai but not in upstream because of language/translation differences.
- **Updates that bring goai closer to upstream** — porting new upstream features, fixing divergences, syncing API changes.

We don't accept:

- **Changes that deviate from upstream's logic or API design**, even if you think they would be improvements. If you have a better idea, take it to [vercel/ai](https://github.com/vercel/ai) first. If it lands upstream, it'll flow into goai naturally.
- **Speculative refactors** that don't track an upstream change.

PRs that diverge from upstream will be politely closed with a pointer to vercel/ai.

## Where to file what

- **Bug reports** — [GitHub Issues](https://github.com/airlockrun/goai/issues). Please include a minimal reproducer.
- **Upstream-tracking PRs** — go ahead and open them; reference the upstream commit/PR in the description.
...
- Write a clear PR description: what changed, why, and how you tested it. For upstream-tracking PRs, link the upstream commit or PR.
