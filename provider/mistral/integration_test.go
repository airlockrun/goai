...
func getProvider() *Provider {
	return New(Options{APIKey: os.Getenv("MISTRAL_API_KEY")})
}
...
func TestIntegration_GenerateText(t *testing.T) {
	...
	t.Run("mistral-small-latest", func(t *testing.T) {
		m := p.LanguageModel("mistral-small-latest")
	...
	t.Run("mistral-large-latest", func(t *testing.T) {
		m := p.LanguageModel("mistral-large-latest")
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
...
func TestIntegration_Embeddings(t *testing.T) {
	skipIfNoKey(t)
	p := getProvider()
	m := p.EmbeddingModel("mistral-embed")
