...
func getProvider() *Provider {
	return New(Options{APIKey: os.Getenv("HUGGINGFACE_API_KEY")})
}
...
func TestIntegration_GenerateText(t *testing.T) {
	...
	t.Run("meta-llama/Meta-Llama-3-8B-Instruct", func(t *testing.T) {
		m := p.LanguageModel("meta-llama/Meta-Llama-3-8B-Instruct")
...
func TestIntegration_Embeddings(t *testing.T) {
	skipIfNoKey(t)
	p := getProvider()
	m := p.EmbeddingModel("sentence-transformers/all-MiniLM-L6-v2")
...
func TestIntegration_ErrorHandling(t *testing.T) {
	skipIfNoKey(t)
	p := getProvider()
	m := p.LanguageModel("no-such-model/no-such-model")
