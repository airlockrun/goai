...
func skipIfNoKey(t *testing.T) {
	if os.Getenv("GOOGLE_VERTEX_PROJECT") == "" {
		t.Skip("GOOGLE_VERTEX_PROJECT not set")
	}
	// Also need either GOOGLE_VERTEX_ACCESS_TOKEN or default credentials
	if os.Getenv("GOOGLE_VERTEX_ACCESS_TOKEN") == "" && os.Getenv("GOOGLE_APPLICATION_CREDENTIALS") == "" {
		t.Skip("GOOGLE_VERTEX_ACCESS_TOKEN or GOOGLE_APPLICATION_CREDENTIALS not set")
	}
}
...
func getProvider() *Provider {
	return New(Options{
		...
		Location:    getEnvOrDefault("GOOGLE_VERTEX_LOCATION", "us-central1"),
...
func TestIntegration_GenerateText(t *testing.T) {
	...
	t.Run("gemini-1.5-flash", func(t *testing.T) {
		m := p.LanguageModel("gemini-1.5-flash")
	...
	t.Run("gemini-1.5-pro", func(t *testing.T) {
...
func TestIntegration_Embeddings(t *testing.T) {
	skipIfNoKey(t)
	p := getProvider()
	m := p.EmbeddingModel("text-embedding-004")
