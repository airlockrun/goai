...
func getProvider() *Provider {
	return New(Options{APIKey: os.Getenv("GROQ_API_KEY")})
}
...
func TestIntegration_GenerateText(t *testing.T) {
	...
	t.Run("llama-3.1-8b-instant", func(t *testing.T) {
		m := p.LanguageModel("llama-3.1-8b-instant")
	...
	t.Run("mixtral-8x7b-32768", func(t *testing.T) {
		m := p.LanguageModel("mixtral-8x7b-32768")
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
...
func TestIntegration_ToolCalls(t *testing.T) {
	skipIfNoKey(t)
	p := getProvider()
	m := p.LanguageModel("llama-3.1-70b-versatile")
